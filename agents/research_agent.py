...
research_agent = LlmAgent(
    ...
    instruction=(
        ...
        "- The message you receive may specify a required currency, e.g. "
        "'Report all prices in QAR.' If so, use EXACTLY that currency for "
        "every crop, converting from whatever your sources quote (use "
        "google_search to find a current exchange rate if needed). If no "
        "currency is specified, pick whichever currency is standard for "
        "real market transactions in this region. Either way, use exactly "
        "ONE currency for every crop in this response — never mix "
